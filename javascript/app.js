  ...
  getPartner(partnerId,cardId)
    .then((partner) => {
      ...
      getHist(partnerId,cardId,'partner').then((resData)=>{
        ...
        getCourse(partnerId,cardId,'partner').then((resData)=>{
          ...
          dat.forEach(v => { 
            var course = {};
            course.title = v.Record.title;
            course.content = v.Record.content;
            course.point = v.Record.point;
            course.key = v.Key;
            allResults.push(course)
          });

          returnData.listCourse = allResults;
          //returnData.earnPointsResult = dat.earnPointsResults;
          //res.json(returnData);
          res.json(returnData);
        });
...
app.post('/api/createCourse', function(req, res) {
  ...
  doCreateCourse(partnerId,cardid,title,content,point).then((response) => {
    if (response.error != null) {
...
async function getCourse(accountNumber,cardId,typeDoc) {
  var returnResult = {};
  ...
  try {
    ...
    // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
    // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
    const result = await contract.submitTransaction('queryCourse');
    console.log(`Transaction has been evaluated, result is aaaaaa : ${result.toString()}`);
...
async function doCreateCourse(partnerId,cardId,title,content,point) {
  var returnResult = {};
  returnResult.error = '';
  ...
  try {
    ...
    // Create a new gateway for connecting to our peer node.
    const gateway = new Gateway();
    await gateway.connect(ccpPath, { wallet, identity: 'partner_'+partnerId+'_'+cardId, discovery: { enabled: true, asLocalhost: true } });
    ...
    const result = await contract.submitTransaction('querySeq'); 
    ...
    for(let val of dat) {
      ...
      console.log("val",val);
      console.log("val.toString",val.Record.course);
      idx = val.Record.course;
    }
    
    await contract.submitTransaction('createCourse',partnerId,cardId,JSON.stringify(course),idx);
    //console.log(`Transaction has been evaluated, result is : ${result.toString()}`);
